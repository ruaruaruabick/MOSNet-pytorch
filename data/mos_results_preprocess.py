...
RESULT_DIR = './data/vcc2018_listening_test_scores'
...
for i in tqdm(range(1,1781)):
    t = str(i).zfill(6)
    new_df = new_df.append({'audio': t+'.wav', 'mean_mos': 5}, ignore_index=True)

new_df.to_csv('./data/mos_list.txt', sep=',', index=False, header=False)
